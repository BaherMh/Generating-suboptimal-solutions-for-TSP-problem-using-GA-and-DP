...
consists to subdivide the individual on equal segments sized k,k=5 for example, and applying DP to compute the shortest path of each segment as follows:
$$
Algorithm_Apply_DP(integer i, p_dp):
Begin:
  Generate probability p_i
  if p_i < p_dp:
    For each segment S of individual i compute shortest path(S)
End
C(S,J)=min┬(iϵ S-{j} )⁡〖C(S-{j},i)+d_(i,j) ∶i∊S,i≠j〗
